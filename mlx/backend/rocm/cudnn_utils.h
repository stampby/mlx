...
class DnnGraph : public fe::graph::Graph {
 public:
  DnnGraph(hipdnnHandle_t handle, Dtype io_dtype, Dtype compute_dtype = float32)
      : handle_(handle) {
  ...
  // Add cuDNN graph to HIP graph, using native HIP graph API.
  fe::error_t encode_graph(
      cu::CommandEncoder& encoder,
      std::unordered_map<int64_t, void*> variant_pack);
  // Add cuDNN graph to HIP graph, using stream capture.
 ...
 private:
  ...
  std::optional<CudaGraph> cached_hip_graph_;
