...
class CudaEventPool {
 ...
 private:
  std::vector<CudaEventHandle>& cache_for(Device& d, int flags) {
    return cache_[d.hip_device()][flags];
  }
...
CudaEvent::CudaEvent(Device& d, int flags)
    : event_(hip_event_pool().create(d, flags)) {}

CudaEvent::~CudaEvent() {
  hip_event_pool().release(std::move(event_));
}
...
AtomicEvent::AtomicEvent(Device& d) {
  void* buf;
  hipError_t (*hip_free)(void*);
  // There are 3 kinds of systems we are implementing for:
  // 1. concurrentManagedAccess == true
  //    => use hip::atom_ref on managed memory
  // 2. hostNativeAtomicSupported == true
  //    => use hip::atom_ref on pinned host memory
  // 2. no hardware cpu/gpu coherency
  //    => use hip::atom_ref on device memory
  d.make_current();
  auto [concurrent_managed_access, host_native_atomic] = check_gpu_coherency();
  if (concurrent_managed_access) {
    CHECK_HIP_ERROR(hipMallocManaged(&buf, sizeof(uint32_t)));
    hip_free = hipFree;
    coherent_ = true;
  } else if (host_native_atomic) {
    CHECK_HIP_ERROR(hipHostMalloc(&buf, sizeof(uint32_t)));
    hip_free = hipHostFree;
    coherent_ = true;
  } else {
    CHECK_HIP_ERROR(hipMalloc(&buf, sizeof(uint32_t)));
    hip_free = hipFree;
    coherent_ = false;
  }
  buf_ = std::shared_ptr<void>(
      buf, [hip_free](void* buf) { CHECK_HIP_ERROR(hip_free(buf)); });
  if (coherent_) {
    *ptr() = 0;
  } else {
    CHECK_HIP_ERROR(hipMemset(buf, 0, sizeof(uint32_t)));
  }
}
...
void AtomicEvent::wait(hipStream_t stream, uint32_t value) {
...
void AtomicEvent::signal(hipStream_t stream, uint32_t value) {
...
uint32_t AtomicEvent::value() const {
  ...
  if (coherent_) {
    ...
    uint32_t val;
    CHECK_HIP_ERROR(
        hipMemcpy(&val, ptr(), sizeof(uint32_t), hipMemcpyDeviceToHost));
    return val;
  }
}
...
struct EventImpl {
  ...
  std::unique_ptr<cu::CopyableCudaEvent> hip;
