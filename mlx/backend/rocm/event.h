...
struct CudaEventHandle : public CudaHandle<hipEvent_t, hipEventDestroy> {
...
class AtomicEvent {
 public:
  ...
  void wait(hipStream_t stream, uint32_t value);
  void wait(Stream s, uint32_t value);
  void signal(uint32_t value);
  void signal(hipStream_t stream, uint32_t value);
