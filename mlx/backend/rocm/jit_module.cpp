...
void check_nvrtc_error(const char* name, hiprtcResult err) {
...
// Return the default path to HIP toolkit.
const std::filesystem::path& default_hip_toolkit_path() {
#if defined(_WIN32)
  static auto cached_path = []() -> std::filesystem::path {
    std::filesystem::path root(
        LR"(C:\Program Files\NVIDIA GPU Computing Toolkit\HIP)");
    for (auto& file : std::filesystem::directory_iterator(root)) {
      if (std::filesystem::exists(file.path() / "include" / "hip/hip_runtime.h")) {
        return file.path();
      }
    }
    return {};
...
#else
  static std::filesystem::path cached_path = "/usr/local/hip";
#endif
  return cached_path;
}
...
const std::vector<std::string>& include_path_args() {
  static std::vector<std::string> cached_args = []() {
    ...
    // Add path to HIP runtime headers, try local-installed python package
    // first and then system-installed headers.
    path = root_dir.parent_path() / "nvidia" / "hip_runtime" / "include";
    if (!std::filesystem::exists(path)) {
      const char* home = std::getenv("HIP_HOME");
      if (!home) {
        home = std::getenv("HIP_PATH");
      }
      path = home ? std::filesystem::path(home) : default_hip_toolkit_path();
      if (!path.empty()) {
        path = path / "include";
      }
      if (path.empty() || !std::filesystem::exists(path)) {
        throw std::runtime_error(
            "Can not find locations of HIP headers, please set environment "
            "variable HIP_HOME or HIP_PATH.");
      }
    }
    args.push_back(fmt::format("--include-path={}", path.string()));
    return args;
...
bool compiler_supports_device_sass(Device& device) {
  ...
  CHECK_NVRTC_ERROR(hiprtcVersion(&nvrtc_major, &nvrtc_minor));
  if (nvrtc_major < 9) {
    return false;
...
void compile(
  ...
  // Create the program
  hiprtcProgram prog;
  CHECK_NVRTC_ERROR(hiprtcCreateProgram(
      &prog,
      source.c_str(),
  ...
  std::unique_ptr<hiprtcProgram, void (*)(hiprtcProgram*)> prog_freer(
      &prog,
      [](hiprtcProgram* p) { CHECK_NVRTC_ERROR(hiprtcDestroyProgram(p)); });
  for (const auto& name : kernel_names) {
    CHECK_NVRTC_ERROR(hiprtcAddNameExpression(prog, name.c_str()));
  }
  ...
  if (compile_result != HIPRTC_SUCCESS) {
    size_t log_size;
    CHECK_NVRTC_ERROR(hiprtcGetProgramLogSize(prog, &log_size));
    std::vector<char> log(log_size + 1, 0);
    CHECK_NVRTC_ERROR(hiprtcGetProgramLog(prog, log.data()));
    throw std::runtime_error(
  ...
  for (const auto& name : kernel_names) {
    const char* mangled;
    CHECK_NVRTC_ERROR(hiprtcGetLoweredName(prog, name.c_str(), &mangled));
  ...
  size_t ptx_size;
  if (use_sass) {
    CHECK_NVRTC_ERROR(hiprtcGetBitcodeSize(prog, &ptx_size));
  } else {
    CHECK_NVRTC_ERROR(hiprtcGetCodeSize(prog, &ptx_size));
  }
  ptx.resize(ptx_size);
  if (use_sass) {
    CHECK_NVRTC_ERROR(hiprtcGetBitcode(prog, ptx.data()));
  } else {
    CHECK_NVRTC_ERROR(hiprtcGetCode(prog, ptx.data()));
  }
}
...
void load_module(
    ...
    const std::vector<std::pair<std::string, std::string>>& ptx_kernels,
    hipModule_t& module_,
    std::unordered_map<std::string, std::tuple<hipFunction_t, bool, uint32_t>>&
        kernels) {
  // Load module.
  char jit_log[4089] = {};
  ...
  hipError_t jit_result = hipModuleLoadDataEx(
      &module_, ptx.data(), std::size(options), options, values);
  if (jit_result != hipSuccess) {
    throw std::runtime_error(
        fmt::format(
...
std::pair<hipFunction_t, uint32_t> JitModule::get_kernel_and_dims(
