...
auto& cudnn_handles_cache() {
  struct CudnnHandle {
    ~CudnnHandle() {
      if (handle) {
        CHECK_CUDNN_ERROR(hipdnnDestroy(handle));
      }
    }
...
void check_cudnn_error(const char* name, hipdnnStatus_t err) {
  if (err != HIPDNN_STATUS_SUCCESS) {
    throw std::runtime_error(
        fmt::format("{} failed: {}.", name, hipdnnGetErrorString(err)));
  }
}
...
hipdnnHandle_t get_cudnn_handle(cu::Device& device) {
  auto& storage = cudnn_handles_cache().at(device.hip_device());
  if (!storage.handle) {
    device.make_current();
    CHECK_CUDNN_ERROR(hipdnnCreate(&storage.handle));
  }
...
fe::error_t DnnGraph::encode_graph(
  ...
  hipdnnSetStream(handle_, encoder.stream());
  auto* workspace_ptr = prepare_workspace(encoder);
  if (!cached_hip_graph_) {
    // First call: populate the HIP graph from the cuDNN execution plan.
    // Also compute and cache the subgraph key to avoid calling
    // hipGraphKernelNodeGetAttribute on every subsequent call (expensive
    // on WDDM where each driver API call has ~40-400us overhead).
    cached_hip_graph_.emplace(encoder.device());
    RETURN_IF_ERROR(populate_hip_graph(
        handle_, variant_pack, workspace_ptr, *cached_hip_graph_));
    std::tie(cached_subgraph_key_, cached_is_updatable_) =
        cu::subgraph_to_key(*cached_hip_graph_);
  } else {
    // Subsequent calls: patch data pointers without re-running kernel setup.
    RETURN_IF_ERROR(update_hip_graph(
        handle_, variant_pack, workspace_ptr, *cached_hip_graph_));
  }
  // Add the cuDNN child graph to the parent HIP graph for batched launch.
  // The pre-computed subgraph key avoids expensive per-node attribute queries.
  encoder.add_graph_node(
      *cached_hip_graph_, cached_subgraph_key_, cached_is_updatable_);
  return {};
}
