...
inline BytesKey<SDPACacheKey> build_sdpa_cache_key(
  ...
  cache_key.pod.device_id = encoder.device().hip_device();
...
auto& sdpa_cache() {
  static thread_local LRUBytesKeyCache<SDPACacheKey, DnnGraph> cache(
      "MLX_HIP_SDPA_CACHE_SIZE", /* default_capacity */ 256);
  return cache;
}

auto& sdpa_backward_cache() {
  static thread_local LRUBytesKeyCache<SDPACacheKey, DnnGraph> cache(
      "MLX_HIP_SDPA_BACKWARD_CACHE_SIZE", /* default_capacity */ 64);
  return cache;
}
...
DnnGraph build_sdpa_graph(
  ...
  graph.select_behavior_notes(
      {fe::BehaviorNote_t::SUPPORTS_HIP_GRAPH_NATIVE_API});
...
bool supports_sdpa_cudnn(
    ...
    bool do_causal,
    Stream s) {
  static bool enabled = env::get_var("MLX_HIP_USE_CUDNN_SDPA", 1);
  if (!enabled) {
    return false;
  }
