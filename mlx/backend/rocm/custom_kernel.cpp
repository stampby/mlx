...
std::string build_kernel(
  ...
  for (int i = 0; i < inputs.size(); ++i) {
    ...
    kernel_source += "    const ";
    kernel_source += dtype_to_hip_type(arr.dtype());
    kernel_source += "* ";
    kernel_source += name;
    kernel_source += ",\n";
  ...
  for (int i = 0; i < output_names.size(); ++i) {
    ...
    kernel_source += "    ";
    kernel_source += dtype_to_hip_type(dtype);
    kernel_source += "* ";
    kernel_source += name;
...
std::vector<array> precompiled_hip_kernel(
    const std::string& name,
...
void CustomKernel::eval_gpu(
  ...
  auto kernel =
      mod.get_kernel(kernel_name, [smem = shared_memory_](hipFunction_t kernel) {
        if (smem > 0 && smem > 48000) {
          cuFuncSetAttribute(
              kernel, HIP_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES, smem);
        }
      });
