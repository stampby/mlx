...
void check_cufft_error(const char* name, hipfftResult err) {
...
struct CuFFTPlan {
  explicit CuFFTPlan(int device_id, hipfftHandle handle, size_t workspace_size)
...
hipDataType input_type(FFTTransformType transform_type) {
...
hipDataType output_type(FFTTransformType transform_type) {
  switch (transform_type) {
    case FFTTransformType::C2C:
    case FFTTransformType::R2C:
      return HIP_C_32F;
...
hipDataType execution_type(FFTTransformType transform_type) {
  switch (transform_type) {
    case FFTTransformType::C2C:
      return HIP_C_32F;
    case FFTTransformType::R2C:
...
int exec_direction(FFTTransformType transform_type, bool inverse) {
  switch (transform_type) {
    case FFTTransformType::C2C:
      return inverse ? HIPFFT_BACKWARD : HIPFFT_FORWARD;
    case FFTTransformType::R2C:
...
std::shared_ptr<CuFFTPlan> get_fft_plan(
  ...
  try {
    CHECK_CUFFT_ERROR(hipfftCreate(&handle));
    CHECK_CUFFT_ERROR(hipfftSetAutoAllocation(handle, 0));
    CHECK_CUFFT_ERROR(hipfftSetStream(handle, encoder.stream()));

    long long plan_n[1] = {n};
    long long inembed[1] = {input_embed(transform_type, n)};
    long long onembed[1] = {output_embed(transform_type, n)};
    CHECK_CUFFT_ERROR(hipfftXtMakePlanMany(
        handle,
        /* rank= */ 1,
        plan_n,
        inembed,
...
void execute_fft(
  ...
  encoder.device().make_current();
  CHECK_CUFFT_ERROR(hipfftSetStream(plan->handle, encoder.stream()));
  auto* workspace = allocate_workspace(encoder, plan->workspace_size);
  CHECK_CUFFT_ERROR(hipfftSetWorkArea(plan->handle, workspace));

  auto capture = encoder.capture_context();
  CHECK_CUFFT_ERROR(hipfftXtExec(
      plan->handle,
      gpu_ptr<void>(in),
      gpu_ptr<void>(out),
