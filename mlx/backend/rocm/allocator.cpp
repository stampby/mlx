...
inline void* unified_malloc(size_t size) {
  ...
  if (supports_managed_memory()) {
    CHECK_HIP_ERROR(hipMallocManaged(&data, size));
  } else {
...
inline void unified_free(void* data) {
  if (supports_managed_memory()) {
    ...
    CHECK_HIP_ERROR(hipHostFree(data));
  }
}

#if HIPRT_VERSION >= 13000
inline hipMemLocation hip_mem_loc(int i) {
  hipMemLocation loc;
...
#else
inline int hip_mem_loc(int i) {
  return i;
}
#endif // HIPRT_VERSION >= 13000
  ...
  if (supports_managed_memory()) {
    ...
    for (int i = 0; i < device_count; ++i) {
      if (device(i).concurrent_managed_access()) {
        auto loc = hip_mem_loc(i);
        CHECK_HIP_ERROR(hipMemAdvise(
            data_, small_pool_size, hipMemAdviseSetAccessedBy, loc));
      }
    }
  }
...
CudaAllocator::CudaAllocator()
    : buffer_cache_(
          ...
          [this](CudaBuffer* buf) { free_hip_buffer(buf); }) {
  size_t free;
  CHECK_HIP_ERROR(hipMemGetInfo(&free, &total_memory_));
  ...
  for (int i = 0; i < device_count; ++i) {
    ...
    if (d.memory_pools()) {
      ...
      CHECK_HIP_ERROR(hipDeviceGetDefaultMemPool(&mem_pools_[i], i));
    }
  }
}

Buffer
CudaAllocator::malloc_async(size_t size, int device, hipStream_t stream) {
  if (size == 0) {
  ...
  if (!buf) {
    ...
    if (!buf) {
      ...
      if (device == -1) {
        ...
        if (mem_pools_[device]) { // supports memory pools
          CHECK_HIP_ERROR(hipMallocAsync(&data, size, stream));
        } else {
          CHECK_HIP_ERROR(hipMalloc(&data, size));
        }
      }
      if (!data) {
    ...
    // If any hip memory pool has too much reserved memory, clear some
    ...
    if (get_cache_memory() > 0) {
      for (auto p : mem_pools_) {
        if (p) {
          size_t used = 0;
          CHECK_HIP_ERROR(hipMemPoolGetAttribute(
              p, hipMemPoolAttrReservedMemCurrent, &used));
...
void CudaAllocator::move_to_unified_memory(
  ...
  hipMemcpyKind kind =
      supports_managed_memory() ? hipMemcpyDefault : hipMemcpyDeviceToHost;
  if (stream && mem_pools_[buf.device]) {
    CHECK_HIP_ERROR(hipMemcpyAsync(data, buf.data, buf.size, kind, stream));
    free_async(buf, stream);
  } else {
    CHECK_HIP_ERROR(hipMemcpy(data, buf.data, buf.size, kind));
    free_async(buf);
  }
  buf.data = data;
  buf.device = -1;
...
void CudaAllocator::free_hip_buffer(CudaBuffer* buf) {
...
void CudaAllocator::free_async(CudaBuffer& buf, hipStream_t stream) {
  if (buf.device == -1) {
    ...
    if (mem_pools_[buf.device]) {
      ...
      CHECK_HIP_ERROR(hipFreeAsync(buf.data, stream));
    } else {
...
Buffer malloc_async(size_t size, CommandEncoder& encoder) {
  return allocator().malloc_async(
      size, encoder.device().hip_device(), encoder.stream());
}
